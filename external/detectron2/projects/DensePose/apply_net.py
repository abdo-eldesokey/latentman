...
class InferenceAction(Action):
    ...
    def _get_input_file_list(cls: type, input_spec: str):
        if os.path.isdir(input_spec):
            file_list = [
                os.path.join(input_spec, fname) for fname in os.listdir(input_spec) if os.path.isfile(os.path.join(input_spec, fname))
            ]
...
class DumpAction(InferenceAction):
    ...
    def execute_on_outputs(cls: type, context: Dict[str, Any], entry: Dict[str, Any], outputs: Instances):
        ...
        img_name = Path(image_fpath).name
        ...
        # Save to .pkl
        pkl_path = os.path.join(context["out_dir"], img_name.replace("jpg", "pkl"))
        with open(pkl_path, "wb") as hFile:
            torch.save(result, hFile)
            logger.info(f"Output saved to {pkl_path}")

    @classmethod
    def create_context(cls: type, args: argparse.Namespace, cfg: CfgNode):
        context = {"results": [], "out_dir": args.output}
...
class ShowAction(InferenceAction):
    ...
    def add_arguments(cls: type, parser: argparse.ArgumentParser):
        ...
        parser.add_argument(
            ...
            help="Comma separated list of visualizations, possible values: " "[{}]".format(",".join(sorted(cls.VISUALIZERS.keys()))),
        )
        parser.add_argument(
            "--min_score",
            metavar="<score>",
