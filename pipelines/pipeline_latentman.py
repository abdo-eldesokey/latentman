...
class StableDiffusionLatentMan(DiffusionPipeline, TextualInversionLoaderMixin, LoraLoaderMixin):
    r"""
    ...
    def __call__(
        ...
        sla_steps: Optional[List] = None,
        pwg_steps: Optional[List] = None,
        rho: float = 0.01,
        reference_mode: str = "prev_new",
        dps_low: Optional[List] = None,
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                if i in sla_steps: # Apply Spatial Latent Alignment                    
                    B = latents.shape[0]
                    for ii in range(1, B):
                        ref_latents = latents[ii - 1].detach().clone() if "prev" in reference_mode else latents[0].detach().clone()
                        x1, y1, x2, y2 = dps_low[64][ii]                        
                        latents[ii, :, y2, x2] = ref_latents[:, y1, x1]
                ...
                with decorator:
                    ...
                    if i in pwg_steps: # Apply Pixel-Wise Guidance                        
                        # Compute x0
                        alpha_prod_t, beta_prod_t, _ = self.scheduler.get_alpha_beta(t)
                        pred_x0 = (latents - beta_prod_t ** (0.5) * noise_pred) / alpha_prod_t ** (0.5)
                        recon = self.vae.decode(1 / 0.18215 * pred_x0)["sample"]  # [-1,1]
                        recon = (recon / 2 + 0.5).clamp(0, 1) * 25
                        B, C, H, W = recon.shape
                        loss = 0
                        recon_ds = F.interpolate(recon, (256, 256), mode="nearest")
                        for ii in range(1, B):
                            ref_recon = recon_ds[ii - 1].detach().clone() if "prev" in reference_mode else recon_ds[0].detach().clone()
                            x1, y1, x2, y2 = dps_low[256][ii]
                            loss += F.mse_loss(recon_ds[ii, :, y2, x2], ref_recon[:, y1, x1])

                        x_in_g = torch.autograd.grad(outputs=loss, inputs=x_in)[0]

                        print(f"{i}: {loss.item():0.06f}, {x_in_g.min().item():0.06f}, {x_in_g.max().item():0.06f}, {x_in_g[0].max().item():0.06f}")

                # compute the previous noisy sample x_t -> x_t-1
                scheduler_out = self.scheduler.step(noise_pred, t, latents, **extra_step_kwargs, return_dict=True)
                pred_x0 = scheduler_out["pred_original_sample"]
                x_prev = scheduler_out["prev_sample"]

                if i in pwg_steps:
                    x_prev = x_prev - rho * x_in_g.chunk(2)[0].detach()
